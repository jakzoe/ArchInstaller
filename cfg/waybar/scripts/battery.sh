...
SUSPEND_FILE=/tmp/batterysuspend
...
# If the battery is low and is not charging (and has not shown notification yet)
    ...
    touch $EMPTY_FILE
elif [ "$BATTERY_LEVEL" -le 15 ] && [ "$BATTERY_DISCHARGING" -eq 1 ]; then
    notify-send "Low Battery" "${BATTERY_LEVEL}% of battery remaining. Suspend when 10% is reached." -u critical -i "battery-alert" -r 9991
fi

if  [ "$BATTERY_LEVEL" -le 10 ] && [ "$BATTERY_DISCHARGING" -eq 1 ]  && [ ! -f $SUSPEND_FILE ]; then
    touch $SUSPEND_FILE 
    systemctl suspend
fi
